...
public class MultiValueFormatterTest {
  ...
  public void testMultiFormatter() {
    ...
    MultiValueFormatter v = new MultiValueFormatter();
    FormattedTabularRecordStreamer.ValueStream<byte[]> valueStream =
        new FormattedTabularRecordStreamer.ValueStream<>(utf8Strings.iterator(), v);
    Assertions.assertEquals("[0-first,0-second]", new String(v.format(valueStream, 0), StandardCharsets.UTF_8));
    Assertions.assertEquals("[1]", new String(v.format(valueStream, 1), StandardCharsets.UTF_8));
    Assertions.assertEquals("[3]", new String(v.format(valueStream, 3), StandardCharsets.UTF_8));
  }
}